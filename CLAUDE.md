...
- **Login de Google**: confirmado funcionando de punta a punta en iOS (Expo Go + QR oficial de Rork, 2026-07-17) **y en Android** (emulador Pixel 8 Pro/Play Store vía dev build local `npx expo run:android`, 2026-07-20). En la prueba de Android, `openAuthSessionAsync` devolvió el code directamente — el fallback `waitForDeepLinkCode` en `useAuth.tsx` (para cuando Chrome Custom Tabs no retorna la URL) no llegó a activarse, así que ese camino sigue sin verificar en un dispositivo físico real. Nota para quien pruebe Android localmente de nuevo: `npx expo run:android` no pasa por el CLI de Rork (`bunx rork start`), así que `EXPO_PUBLIC_PROJECT_ID` no se inyecta solo — hay que agregarlo a mano en `expo/.env` (`EXPO_PUBLIC_PROJECT_ID=fse076rw9ejy1s1vel0nq`) o el deep-link scheme cae a un placeholder (`myapp://`) que no coincide con las Redirect URLs de Supabase y el login termina en `localhost:3000`. Producción (vía Rork) no se ve afectada por esto.
